...
model_yes <- lm(antigenyes ~ day + antigen_result, data = true_data)
model_bye <- lm(antigenbye ~ day + antigenyes, data = true_data)
model_day <- lm(day ~ antigen_result + antigenyes + antigenbye, data = true_data)
true_data <- true_data |>
  drop_na(antigenyes, antigenbye) |>
  mutate(dayhat = fitted(model_day))
ggplot(true_data, aes(x = antigen_result, y = dayhat)) + 
  geom_point() +
  geom_line()
```
